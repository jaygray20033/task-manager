...
function renderTasks() {
  ...
  if (currentCategory !== "all") {
    filteredTasks = filteredTasks.filter((t) => (t.category || "Chung") === currentCategory)
  }
  ...
  taskList.innerHTML = filteredTasks
    .map(
      (task) => `
        <div class="task-item ${task.completed ? "completed" : ""} ${task.isImportant ? "important" : ""}" data-id="${task._id}">
            <input 
                type="checkbox" 
                class="task-checkbox" 
            ...
            <div class="task-content">
                <div class="task-description">
                    ${task.isImportant ? '<span class="important-badge">⭐</span>' : ""}
                    ${escapeHtml(task.description)}
                    ${task.dueDate ? getDueDateBadge(task.dueDate, task.completed) : ""}
                </div>
                <div class="task-meta">
                    <span class="task-category-badge">${escapeHtml(task.category || "Chung")}</span>
                    Tạo lúc: ${formatDate(task.createdAt)}
                    ${task.dueDate ? ` • Hẹn lúc: ${formatDate(task.dueDate)}` : ""}
                </div>
            </div>
            <div class="task-actions">
                <button 
                    class="task-btn task-btn-important ${task.isImportant ? "active" : ""}" 
                    onclick="toggleImportant('${task._id}')"
                    title="${task.isImportant ? "Bỏ đánh dấu quan trọng" : "Đánh dấu quan trọng"}"
                >
                    ${task.isImportant ? "★" : "☆"}
                </button>
                <button class="task-btn task-btn-delete" onclick="deleteTask('${task._id}')">
                    Xóa
                </button>
            </div>
        </div>
      ...
      return `
        <div class="upcoming-task-item">
          <div class="upcoming-task-info">
            <div class="upcoming-task-description">${escapeHtml(task.description)}</div>
            <div class="task-meta">
              <span class="task-category-badge">${escapeHtml(task.category || "Chung")}</span>
